...
#   - APP_NAME              App name for Docker images (default: app)
...
# Per-build identifier. CNB resolves `${APP_NAME}:latest` to a digest and
...
cleanup() { docker rm -f "${APP_NAME}-build-container" 2>/dev/null || true; }
trap cleanup EXIT
...
docker build --platform "linux/${ARCH}" \
    ...
    -f "${SCRIPT_DIR}/Dockerfile.build" -t "${APP_NAME}-build" "${SCRIPT_DIR}" || {
...
docker run --rm --platform "linux/${ARCH}" \
    -v "${BUILD_OUTPUT_DIR}:/output-mount" "${APP_NAME}-build" || {
...
docker build --platform "linux/${ARCH}" \
    ...
    -f "${BUILD_OUTPUT_DIR}/Dockerfile" -t "${APP_NAME}" "${BUILD_OUTPUT_DIR}" || {
...
if [ "${TEST_RUN:-0}" = "1" ]; then
    ...
    TEST_CONTAINER="${APP_NAME}-test-$(date +%s)"
    ...
    docker run -d --name "${TEST_CONTAINER}" -p "${PORT}:${PORT}" -e "PORT=${PORT}" "${APP_NAME}"
...
log_info "Done! Run: docker run -p ${PORT}:${PORT} ${APP_NAME}"
